...
namespace simple_texture {
    ...
    void App::run() {
        std::string name = "Texture Application";
    ...
    std::unique_ptr<fillcan::Model> App::createCubeModel(glm::vec3 offset) {
        std::vector<fillcan::Model::Vertex> vertices{
            ...
            // left face (white)
            {{-.5f, -.5f, -.5f}, {.9f, .9f, .9f}, {0.0f, 0.0f}}, // 0
            {{-.5f, .5f, .5f}, {.9f, .9f, .9f}, {1.0f, 1.0f}},   // 1
            {{-.5f, -.5f, .5f}, {.9f, .9f, .9f}, {0.0f, 1.0f}},  // 2

            {{-.5f, .5f, -.5f}, {.9f, .9f, .9f}, {1.0f, 0.0f}}, // 3

            // right face (yellow)
            {{.5f, -.5f, -.5f}, {.8f, .8f, .1f}, {0.0f, 0.0f}}, // 4
            {{.5f, .5f, .5f}, {.8f, .8f, .1f}, {1.0f, 1.0f}},   // 5
            {{.5f, -.5f, .5f}, {.8f, .8f, .1f}, {0.0f, 1.0f}},  // 6

            {{.5f, .5f, -.5f}, {.8f, .8f, .1f}, {1.0f, 0.0f}}, // 7

            // top face (orange)
            {{-.5f, -.5f, -.5f}, {.9f, .6f, .1f}, {0.0f, 0.0f}}, // 8
            {{.5f, -.5f, .5f}, {.9f, .6f, .1f}, {1.0f, 1.0f}},   // 9
            {{-.5f, -.5f, .5f}, {.9f, .6f, .1f}, {0.0f, 1.0f}},  // 10

            {{.5f, -.5f, -.5f}, {.9f, .6f, .1f}, {1.0f, 0.0f}}, // 11

            // bottom face (red)
            {{-.5f, .5f, -.5f}, {.8f, .1f, .1f}, {0.0f, 0.0f}}, // 12
            {{.5f, .5f, .5f}, {.8f, .1f, .1f}, {1.0f, 1.0f}},   // 13
            {{-.5f, .5f, .5f}, {.8f, .1f, .1f}, {0.0f, 1.0f}},  // 14

            {{.5f, .5f, -.5f}, {.8f, .1f, .1f}, {1.0f, 0.0f}}, // 15

            // nose face (blue)
            {{-.5f, -.5f, 0.5f}, {.1f, .1f, .8f}, {0.0f, 0.0f}}, // 16
            {{.5f, .5f, 0.5f}, {.1f, .1f, .8f}, {1.0f, 1.0f}},   // 17
            {{-.5f, .5f, 0.5f}, {.1f, .1f, .8f}, {0.0f, 1.0f}},  // 18

            {{.5f, -.5f, 0.5f}, {.1f, .1f, .8f}, {1.0f, 0.0f}}, // 19

            // tail face (green)
            {{-.5f, -.5f, -0.5f}, {.1f, .8f, .1f}, {0.0f, 0.0}}, // 20
            {{.5f, .5f, -0.5f}, {.1f, .8f, .1f}, {1.0f, 1.0f}},  // 21
            {{-.5f, .5f, -0.5f}, {.1f, .8f, .1f}, {0.0f, 1.0f}}, // 22

            {{.5f, -.5f, -0.5f}, {.1f, .8f, .1f}, {1.0f, 0.0f}}, // 23
    ...
    void App::loadGameObjects() {
        ...
        spModel1->texture = this->upFillcan->getAssetManager()->getTexture(0);

        std::shared_ptr<fillcan::Model> spModel2 = this->createCubeModel({0.f, 0.f, 0.f});
        spModel2->texture = this->upFillcan->getAssetManager()->getTexture(1);
