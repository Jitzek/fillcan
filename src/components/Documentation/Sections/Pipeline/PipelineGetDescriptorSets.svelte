...
<Highlight
    ...
    code={`std::vector<DescriptorSet*>& getDescriptorSets();`}
/>
<MethodDescription>
    <span slot="details">
        Get all the <Reference sectionID={SectionID.DESCRIPTOR_SET}
            >descriptor sets</Reference
        > of this pipeline.
    </span>
    <span slot="return">
        A list of pointers to all the <Reference
            sectionID={SectionID.DESCRIPTOR_SET}>descriptor sets</Reference
        > of this <Reference sectionID={SectionID.PIPELINE}>pipeline</Reference
        >.
    </span>
</MethodDescription>
