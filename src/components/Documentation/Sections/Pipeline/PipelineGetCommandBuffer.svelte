...
<Highlight
    ...
    code={`CommandBuffer* getCommandBuffer();`}
/>
<MethodDescription>
    <span slot="details">
        Get the bound <Reference sectionID={SectionID.COMMAND_BUFFER}
            >command buffer</Reference
        >.
    </span>
    <span slot="return">
        A pointer to the bound <Reference sectionID={SectionID.COMMAND_BUFFER}
            >command buffer</Reference
        >, or <code>nullptr</code> if no <Reference
