...
<Highlight
    ...
    code={`Queue* getComputeQueue();`}
/>
<MethodDescription>
    <span slot="details">
        Get a pointer to the compute <Reference sectionID={SectionID.QUEUE}
            >Queue</Reference
        >.
    </span>
    <span slot="return">
        A pointer to the compute <Reference sectionID={SectionID.QUEUE}
            >Queue</Reference
