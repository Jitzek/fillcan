...
<Highlight
    ...
    code={`void setSharingMode(VkSharingMode sharingMode);`}
/>
<MethodDescription>
    <span slot="details">
        Set the sharing mode for the <Reference sectionID={SectionID.BUFFER}
            >Buffer</Reference
        >.
    </span>
    <div slot="params">
        <li>
            ...
            > supported by the Device. Because Vulkan can run many commands in parallel,
            some implementations need to know whether the <Reference
                sectionID={SectionID.BUFFER}>Buffer</Reference
            > will essentially be used by a single command at a time or possibly
            by several. Setting
            <code>sharingMode</code>
            to <code>VK_SHARING_MODE_EXCLUSIVE</code> tells Vulkan that the
            <Reference sectionID={SectionID.BUFFER}>Buffer</Reference> will only
            be used for a single <Reference sectionID={SectionID.QUEUE}
                >Queue</Reference
            >, while setting
            <code>sharingMode</code>
            to
            <code>VK_SHARING_MODE_CONCURRENT</code> specifies that the
            application will use the <Reference sectionID={SectionID.BUFFER}
                >Buffer</Reference
            > on multiple <Reference sectionID={SectionID.QUEUE}
                >Queues</Reference
            > at once. Using
